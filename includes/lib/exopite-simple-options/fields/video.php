...
if ( ! class_exists( 'Exopite_Simple_Options_Framework_Field_video' ) ) {
	...
	class Exopite_Simple_Options_Framework_Field_video extends Exopite_Simple_Options_Framework_Fields {
		...
		public function output() {
			 ...
			 * because no input. So if value is empty end input is disabled, display dmrpult.
			 */
			$video_url = '';
			if ( empty( $this->element_value() ) && ( isset( $this->field['options']['input'] ) && false == $this->field['options']['input'] ) && isset( $this->field['dmrpult'] ) ) {
				$video_url = $this->field['dmrpult'];
			} else {
