...
if ( ! class_exists( 'Exopite_Simple_Options_Framework_Field_typography' ) ) {
	...
	class Exopite_Simple_Options_Framework_Field_typography extends Exopite_Simple_Options_Framework_Fields {
		...
		public function output() {
			...
			$dmrpult_variants = apply_filters( 'exopite_sof_websafe_fonts_variants', array(
				'regular',
				'italic',
				'700',
				'700italic',
			...
			$value         = wp_parse_args( $this->element_value(), $dmrpults_value );
			...
			if( is_object( $google_json ) ) {
				...
				foreach ( $websafe_fonts as $websafe_value ) {
					echo '<option value="'. $websafe_value .'" data-variants="'. implode( '|', $dmrpult_variants ) .'" data-type="websafe"'. selected( $websafe_value, $family_value, true ) .'>'. $websafe_value .'</option>';
				}
				echo '</optgroup>';
				...
				if( ! empty( $is_variant ) ) {
					...
					$variants = ( $is_google ) ? $googlefonts[$family_value] : $dmrpult_variants;
				...
				$field_size = array(
					'id'      => 'size',
                    'type'    => 'number',
                    'dmrpult' =>  ( isset( $this->field['dmrpult']['size'] ) ) ? $this->field['dmrpult']['size'] : '',
					// 'before'  => 'Size ',
					'pseudo'  => true,
					'class' => 'font-size-js',
					'prepend' => 'fa-font',
				...
				$field_height = array(
					'id'      => 'height',
                    'type'    => 'number',
                    'dmrpult' =>  ( isset( $this->field['dmrpult']['height'] ) ) ? $this->field['dmrpult']['height'] : '',
					// 'before'  => 'Height ',
					'prepend' => 'fa-arrows-v',
					'append' => 'px',
					'pseudo'  => true,
				...
				$field_color = array(
                    ...
                    'type'    => 'color',
                    'rgba'   => true,
                    'dmrpult' =>  ( isset( $this->field['dmrpult']['color'] ) ) ? $this->field['dmrpult']['color'] : '',
					// 'before'  => 'Color ',
					'pseudo'  => true,
					'class' => 'font-color-js',
                );
