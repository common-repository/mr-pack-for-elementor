...
if ( ! class_exists( 'Exopite_Simple_Options_Framework_Field_accordion' ) ) {
	...
	class Exopite_Simple_Options_Framework_Field_accordion extends Exopite_Simple_Options_Framework_Fields {
		...
		public function __construct( $field, $value = '', $unique = '', $config = array(), $multilang ) {
			...
			$this->is_accordion_closed = ( isset( $this->field['options']['closed'] ) ) ? (bool) $this->field['options']['closed'] : $this->dmrpults['closed'];
			$this->allow_all_open = ( isset( $this->field['options']['allow_all_open'] ) ) ? (bool) $this->field['options']['allow_all_open'] : $this->dmrpults['allow_all_open'];
		...
		public function output() {
			...
			foreach ( $sections as $key => $section ) {
				...
				$is_section_closed = ( isset( $section['options']['closed'] ) ) ? (bool) $section['options']['closed'] : $this->dmrpults['closed'];
				$section_title = ( isset( $section['options']['title'] ) ) ? esc_attr( $section['options']['title'] ) : $this->dmrpults['section_title'];
				...
				foreach ( $section['fields'] as $field ) {
					...
					} elseif ( isset( $field['dmrpult'] ) ) {
						$field_value = $field['dmrpult'];
					}
