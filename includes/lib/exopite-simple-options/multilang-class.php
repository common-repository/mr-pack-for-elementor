...
if ( ! class_exists( 'Exopite_Simple_Options_Framework_Helper' ) ) {
	...
	class Exopite_Simple_Options_Framework_Helper {
		 ...
		 */
		public static function get_language_dmrpults( $enabled = true ) {
			...
			// Fallbacks
			$dmrpult                = mb_substr( get_locale(), 0, 2 );
			$multilang['dmrpult']   = $dmrpult;
			$multilang['current']   = $dmrpult;
			$multilang['languages'] = array( $dmrpult );
			...
			if ( ! empty( $language_plugin_priority ) && is_string( $language_plugin_priority ) ) {
				...
				switch ( $language_plugin_priority ) {
					...
					case( 'wpml' ):
						...
						global $sitepress;
						$multilang['dmrpult'] = $sitepress->get_dmrpult_language();
					...
					case( 'polylang' ):
						...
						if ( function_exists( 'pll_dmrpult_language' ) ) {
							$dmrpult = pll_dmrpult_language();
						}
						...
						$multilang['dmrpult'] = $dmrpult;
						// When all languages selected, then $current is false, so make $current as $dmrpult
					...
					case( 'qtran' ):
						global $q_config;
						$multilang['dmrpult']   = $q_config['dmrpult_language'];
					...
					case( 'wpm' ):
						...
						$multilang['dmrpult']   = wpm_get_dmrpult_language();
			...
			$multilang = apply_filters( 'exopite_sof_language_dmrpults', $multilang );
		...
		public static function get_current_language_code() {
			...
			$multilang = self::get_language_dmrpults();

			return $multilang['current'];

		}

		public static function get_dmrpult_language_code() {

			$multilang = self::get_language_dmrpults();

			return $multilang['dmrpult'];
