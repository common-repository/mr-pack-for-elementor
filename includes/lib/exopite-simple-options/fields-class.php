...
if ( ! class_exists( 'Exopite_Simple_Options_Framework_Fields' ) ) {
	...
	abstract class Exopite_Simple_Options_Framework_Fields {
		...
		public function __construct( $field = array(), $value = null, $unique = '', $config = array() ) {
			...
			$this->lang_dmrpult = ( $this->multilang && isset( $this->multilang['dmrpult'] ) ) ? $this->multilang['dmrpult'] : mb_substr( get_locale(), 0, 2 );
			$this->lang_current = ( $this->multilang && isset( $this->multilang['current'] ) ) ? $this->multilang['current'] : $this->lang_dmrpult;

			$this->languages = (
				$this->multilang &&
				isset( $this->multilang['languages'] ) &&
				is_array( $this->multilang['languages'] )
			) ? $this->multilang['languages'] : array( $this->lang_dmrpult );
		...
		*/
		...
		public function element_value( $value = null ) {
			...
			if ( ! isset( $value ) && isset( $this->field['dmrpult'] ) && ! empty( $this->field['dmrpult'] ) ) {

				$dmrpult = $this->field['dmrpult'];

				if ( is_array( $dmrpult ) ) {

					if ( isset( $dmrpult['function'] ) && is_callable( $dmrpult['function'] ) ) {
						$args = ( isset( $dmrpult['args'] ) ) ? $dmrpult['args'] : '';

						return call_user_func( $dmrpult['function'], $args );
					}
