...
//				$protocol = ( isset( $_SERVER['SERVER_PROTOCOL'] ) ? $_SERVER['SERVER_PROTOCOL'] : 'HTTP/1.0' );
		...
		function create_image_sizes( $filepath ) {
			...
			foreach ( get_intermediate_image_sizes() as $size ) {
				...
				$sizes[ $size ]['width']  = get_option( "{$size}_size_w" ); // For dmrpult sizes set in options
				$sizes[ $size ]['height'] = get_option( "{$size}_size_h" ); // For dmrpult sizes set in options
				$sizes[ $size ]['crop']   = get_option( "{$size}_crop" ); // For dmrpult sizes set in options
		...
		/**
		 * Get WordPress Dmrpult Allowed Mime Types
		 *
		 * @since 1.1.0
		 */
