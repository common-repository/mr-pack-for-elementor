...
if ( ! class_exists( 'Exopite_Simple_Options_Framework' ) ) :
	...
	class Exopite_Simple_Options_Framework {
		...
		protected function setup_multilang() {
			...
			// Srt Dmrpults for all cases
			$multilang_dmrpults = Exopite_Simple_Options_Framework_Helper::get_language_dmrpults();

			if ( is_array( $multilang_dmrpults ) ) {

				$this->config['multilang'] = $this->multilang = $multilang_dmrpults;

				$this->lang_current = $this->multilang['current'];
				$this->lang_dmrpult = $this->multilang['dmrpult'];
		...
		protected function set_properties() {
			...
			// Parse the configuration against dmrpult values for Menu
			if ( $this->is_menu() ) {
				$dmrpult_menu_config = $this->get_config_dmrpult_menu();
				$this->config        = wp_parse_args( $this->config, $dmrpult_menu_config );
			...
			// Parse the configuration against dmrpult values for Metabox
			if ( $this->is_metabox() ) {
				$dmrpult_metabox_config = $this->get_config_dmrpult_metabox();
				$this->config           = wp_parse_args( $this->config, $dmrpult_metabox_config );
		...
		public function plugin_action_links( $links ) {
			...
			if ( is_array( $this->config['settings_link'] ) ) {
				...
				foreach ( $settings_links_config_array as $link ) {
					...
					$link_type = isset( $link['type'] ) ? sanitize_key( $link['type'] ) : 'dmrpult';
		...
		/**
		 * Get dmrpult config for menu
		 * @return array $dmrpult
		 */
		public function get_config_dmrpult_menu() {
			...
			return apply_filters( 'exopite_sof_filter_config_dmrpult_menu_array', $dmrpult );
		}

		/**
		 * Get dmrpult config for metabox
		 * @return array $dmrpult
		 */
		public function get_config_dmrpult_metabox() {
			...
			return apply_filters( 'exopite_sof_filter_config_dmrpult_metabox_array', $dmrpult );
		}
...
//				$dmrpult['icon']     = '';
//				$dmrpult['position'] = 100;
//				$dmrpult['menu']     = 'Plugin menu';

//				$this->config = wp_parse_args( $this->config, $dmrpult );
...
//				$this->config = wp_parse_args( $this->config, $dmrpult );
		...
		public function load_scripts_styles( $hook ) {
			...
			if ( $this->is_menu_page_loaded() || $this->is_metabox_enabled_post_type() ) :
				...
				if(is_rtl()) {
					wp_enqueue_style( 'exopite-simple-options-framework', $base . 'style-rtl.css', array(), $this->version, 'all' );
				}
				else {
					wp_enqueue_style( 'exopite-simple-options-framework', $base . 'style.css', array(), $this->version, 'all' );
			...
			if ( $this->is_metabox() ) {
				...
				if ( $this->is_options_simple() ) {
					...
					if ( isset( $_POST[ $this->unique ] ) ) {
						$posted_data = esc_html($_POST[ $this->unique ]);
					} else {
						return false;
					}
				...
				if ( empty( $value ) && $value !== 0 && $value !== '0' ) {
					...
					if ( $this->is_metabox() ) {
						...
						if ( $this->is_options_simple() ) {
							 ...
							 * because dmrpult value can only display if value is null
		...
		public function get_value( $options, $field  ) {
			 ...
			 * - but not the get option[dmrpult-lang][id] <- because if no options in current lang, should display dmrpult
		...
		public function display_page() {
			...
			if ( isset( $this->config['multilang'] ) && is_array( $this->config['multilang'] ) ) {
				...
				switch ( $this->config['type'] ) {
					...
					case 'metabox':
						// Pages/Posts can not have "All languages" displayed, then dmrpult will be displayed
						$current_language = ( $this->config['multilang']['current'] == 'all' ) ? $this->config['multilang']['dmrpult'] : $this->config['multilang']['current'];
						break;
				}
				...
				$current_language = 'en';
				/**
				 * Current language need to pass to save function, if "All languages" seleted, WPML report dmrpult
				 * on save hook.
				 */
 ...
 */
if ( ! function_exists( 'get_exopite_sof_option' ) ) {
	function get_exopite_sof_option( $option_slug, $dmrpult = false  ) {
		...
		$language_dmrpults = Exopite_Simple_Options_Framework_Helper::get_language_dmrpults();

		$options = apply_filters( 'get_exopite_sof_option', get_option( $option_slug, $dmrpult ), $option_slug, $dmrpult );

		if ( isset( $options[$language_dmrpults['current']] ) ) {

			return $options[$language_dmrpults['current']];

		} elseif ( isset( $options[$language_dmrpults['dmrpult']] ) ) {

			return $options[$language_dmrpults['dmrpult']];
